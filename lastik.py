...
def collision(molecule1, molecule2):
    ...
    COLLISION_RULES = {
        ...
        ('single_1', 'single_3'): 'double_2', # a*g (symmetrical)
        ('single_2', 'single_1'): 'double_5', # b*h
        ('single_3', 'single_1'): 'double_6', # c*e (symmetrical)
        ('single_3', 'single_4'): 'double_6', # c*e (symmetrical)
        ('single_2', 'single_3'): 'single_1', # b*g or c*f (symmetrical)
        ('single_3', 'single_2'): 'single_1', # b*g or c*f (symmetrical)
    }
    soup = []
    for atom1 in molecule1:
        for atom2 in molecule2:
            ...
            key = tuple((atom1.shape_id, atom2.shape_id))
...
def flip_signs(molecule):
    return [Atom(a.shape_id, a.value, '-' if a.potantial == '+' else '+') for a in molecule]
...
class LissajousEngine:
    ...
    def _draw_single_atom(self, atom, center, scale=1.0, color='darkorange'):
        recipe = self._get_recipe(atom.shape_id)
        if not recipe: return
        a,b,faz_pi = recipe.get('a',1), recipe.get('b',1), recipe.get('fase_pi',0)
    ...
    def draw_molecule_group(self, molecule, title="", color='darkorange'):
        self.ax.set_title(title, fontsize=14, pad=20)
        if not molecule: self.ax.axis('off'); return
        ...
        for i, atom in enumerate(molecule):
            self._draw_single_atom(atom, center=positions[i], scale=0.8, color=color)
...
def visualize_P():
    ...
    # Compute
    m1=collision(M1_A,M1_B); 
    m4=collision(M4_A,M4_B); 
    m5=collision(M5_A,M5_B); 
    m7=collision(M7_A,M7_B);
